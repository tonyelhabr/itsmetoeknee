...
brier_skill_score.data.frame <- function(data, truth, estimate, ref_estimate, ...) {
  yardstick::metric_summarizer(
...
I should note that the `{probably}` package offers `cal_validate_*()` functions to help with deciphering whether a calibrated model is truly better than the original. That set of functions uses a more robust and valid technique that I have here.[^6]

[^6]: The `cal_validate_*()` function resample a holdout set, compute ROC AUC and Brier score, and calculate standard error of the performance metrics.

```{r}
#| label: cal_validate_beta
#| code-fold: true
#| code-summary: Using probably::cal_validate_beta

set.seed(42)
sets <- rsample::vfold_cv(matches)

fixed_brier_skill_score <- purrr::partial(
  brier_skill_score,
  ref_estimate = !!ref_estimate
)
...
# debugonce(tune:::collect_predictions.tune_results)
segment_logistic %>%
  rsample::vfold_cv() %>%
  cal_validate_beta(Class, save_pred = TRUE) |> 
  collect_predictions()
```
