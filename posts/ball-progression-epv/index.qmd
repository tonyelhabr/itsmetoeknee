...
example_export_df %>%
  ...
  purrr::iwalk(
    \(.x, .y) {
      .x |>
        dplyr::select(-pass_status) |> 
        dplyr::mutate(value = prop) |> 
        jsonlite::write_json(
          file.path(PROJ_DIR, sprintf('%s_actual_prop_data.json', .y)),
          pretty = TRUE,
        )
    }
  )
...
#| include: false
#| eval: false
plot_pass_grid <- function(x, y, f, pass_type = 'pass', ...) {
  df <- export_df |> 
    dplyr::filter(pass_type == .env$pass_type) |> 
    dplyr::transmute(
      pass_status,
      n,
      prop,
...
export_df |> 
  filter(idx1 == '13', idy1 == '01') |> 
  group_by(pass_type, pass_status) |> 
  summarize(
    n = sum(n, na.rm = TRUE)
  )
```
...
::: {.callout-note title=""Typical" and ""Hypothetical" collapse="false"}

**Typical**: I've selected "constant" values for non-spatial features, i.e. game state of 0-0 with no time elapsed, pass taken with foot.
...
The gradient in the plot below highlights the relative frequency of successful passes from the center spot more directly.

```{ojs}
//| include: true
//| fig-cap: A heatmap showing the relative frequency of actual successful passes from the center spot (annotated in blue) to all areas on the pitch.
{
  const chart = d3.create("div").style("background-color", "8f8f8f")
...
//| output: false
{
  ...
  d3.select("#heatmap-complete-empirical-prop")
    .html("")
    .datum(complete_actual_prop_data)
...
//| output: false
{
  ...
  d3.select("#heatmap-incomplete-empirical")
    .html("")
    .datum(incomplete_actual_pv_data)
...
```{ojs}
//| label: complete_actual_prop_data
//| include: false
complete_actual_prop_data  = FileAttachment("complete_actual_prop_data.json").json()
```
