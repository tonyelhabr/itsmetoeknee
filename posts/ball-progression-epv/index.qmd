...
undummify <- function(xy, rgx) {
  ...
  xy |> 
    ...
    pivot_longer(starts_with(rgx)) |> 
    filter(value == 1L) |> 
    mutate(name = stringr::str_remove_all(name, rgx2))
}

types <- undummify(xy, 'type_') |> 
  select(
    all_of(MODEL_ID_COLS),
    type_name = name
  )

results <- undummify(xy, 'result_') |> 
  select(
    all_of(MODEL_ID_COLS),
    result_name = name
  )
...
#| include: false
#| eval: false
qs::qsave(vaep, file.path(PROJ_DIR, 'vaep.qs'))
```
...
#| include: false
#| eval: false
vaep <- qs::qread(file.path(PROJ_DIR, 'vaep.qs'))
```
...
#| include: false
#| eval: false
export_nested_pv_grid <- function(col, name) {
    ...
    purrr::iwalk(
      \(.x, .y) {
        ...
        dplyr::transmute(
          ...
          dplyr::arrange(j, i)  |> 
          jsonlite::write_json(
            file.path(PROJ_DIR, sprintf('%s_empirical_nested_%s_data.json', .y, name)),
            pretty = TRUE,
            auto_unbox = TRUE
          )
      }
...
VAEP directly reflects the value added by an action relative to the prior action. For those who have worked with expected threat (xT) before, this is analogous to [the "xT created" metric](https://karun.in/blog/expected-threat.html), as described by Singh.
...
Compared to the prior dynamic pitch for incomplete passes, this one looks visually similar for most start-end location pairs. Perhaps this is not surprising, as our pre-Appendix pitches were accounting for the probability of conceding already. Accounting for the value of the pre-pass state only seems to re-enforce the incomplete pass trends we observed before--an incomplete pass can have a positive impact, but only really in cases in which the ball is passed into the box or near the top of the box.
