...
pv_grid <- mini_preds_and_features |> 
  dplyr::left_join(
...
x_width <- diff(X_SEQ)[1]
y_width <- diff(Y_SEQ)[1]
as.integer(EXAMPLE_IDX1) * x_width
as.integer(EXAMPLE_IDY1) * y_width
    ...
    purrr::iwalk(
      \(.x, .y) {
        .x |>
          ...
          jsonlite::write_json(
            file.path(PROJ_DIR, sprintf('%s_empirical_%s_data.json', .y, name)),
            pretty = TRUE
          )
      }
    )
...
{  
  const heatmap_complete_empirical_nested = d3_soccer.heatmap(pitch)
    ...
    .onSelect((x, y, v) => {
      ...
      const updatedSvg = d3.select("#heatmap-complete-empirical-nested").select("svg");

      const updatedCells = updatedSvg.selectAll(".cell");
      
      updatedCells.each(function(d, i) {
        const cell = d3.select(this);
        const bbox = this.getBBox();
