...
select_np_season_shooting <- player_season_np_shots |>
  dplyr::filter(player %in% unname(PLAYERS_OF_INTEREST))
...
select_np_season_shooting |> 
  ...
  dplyr::inner_join(
    select_np_season_shooting |> 
      dplyr::filter(season_end_year == LATEST_SEASON_END_YEAR) |> 
      dplyr::select(player_id, latest_o = o),
    by = dplyr::join_by(player_id)
  ) |> 
  dplyr::mutate(
...
#> # A tibble: 2 × 7
#>   player_id player          mean    sd latest_o z_score p_value
...
select_resampled_shots <- purrr::imap_dfr(
  ...
  \(.player, .player_id) {
    ...
    latest_shots <- player_shots |>
      dplyr::filter(season_end_year == LATEST_SEASON_END_YEAR)
    
    player_shots |>
      dplyr::filter(season_end_year < LATEST_SEASON_END_YEAR) |> 
      resample_player_shots(
        n_shots_to_sample = nrow(latest_shots)
      ) |> 
      dplyr::mutate(
        player_id = .player_id,
        player = .player
...
select_resampled_shots_with_latest_o <- select_resampled_shots |> 
  dplyr::inner_join(
    player_season_np_shots |> 
      dplyr::filter(season_end_year == LATEST_SEASON_END_YEAR) |> 
      dplyr::select(
...
select_resampled_props <- select_resampled_shots_with_latest_o |> 
  dplyr::summarize(
    .by = c(player_id, player),
    prop_lte = sum(o <= latest_o) / n(),
    prop_gte = sum(o >= latest_o) / n()
  ) |> 
  dplyr::arrange(player)
...
maddison_resampled_shots_with_latest_o <- select_resampled_shots_with_latest_o |>
  dplyr::filter(player == 'James Maddison') |> 
  dplyr::mutate(
    group = ifelse(o <= latest_o, 'lte', 'other')
  )

maddison_resampled_shots_plot <- maddison_resampled_shots_with_latest_o |> 
  ggplot2::ggplot() +
  ggplot2::aes(
    x = o
  ) +
...
init_player_np_shots <- player_season_np_shots |> 
  dplyr::mutate(is_latest = season_end_year == LATEST_SEASON_END_YEAR) |> 
  dplyr::summarize(
    .by = c(is_latest, player_id, player),
    dplyr::across(
      c(shots, g, xg),
      \(.x) sum(.x, na.rm = TRUE)
    )
  ...
  dplyr::mutate(o = g / xg) |> 
  dplyr::arrange(player, player_id, is_latest)

## Player must have
## 1. shots in latest season
## 2. more shots in all prior seasons combined than latest season
eligible_players <- init_player_np_shots |> 
  dplyr::transmute(
    player_id,
    latest = ifelse(is_latest, 'latest', 'prior'),
    shots
  ) |> 
  tidyr::pivot_wider(
    names_from = latest,
  ...
  dplyr::filter(latest > 0, prior > latest)
  
player_np_shots <- init_player_np_shots |> 
  dplyr::semi_join(
    eligible_players,
    by = dplyr::join_by(player_id)
  )
...
latest_player_np_shots <- player_np_shots |>
  dplyr::filter(is_latest)
...
```{r}
prop_lte_hist <- prop_sims_with_latest_actual |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = prop_lte
  ) +
