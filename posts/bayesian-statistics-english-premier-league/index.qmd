...
file_path_proj <- partial(file_path, ... = )
file_path_out <- partial(file_path_proj, ... = )
```
...
```{r}
#| label: scrape-prep
path_epl_data <- file_path_proj('epl.xlsx')
```

```{r}
#| label: scrape-appendix-1
# Data Collection
seasons <- 2016L:2018L
```

```{r}
#| label: scrape-appendix-2
...
```{r}
#| label: res_sim_1_export
write_rds(res_sim_1, path_res_sim_1)
```

```{r}
#| label: res_sim_1_import
...
```{r}
#| label: res_summ_1-show-pre
.opt_old <- getOption('tibble.print_min')
options(tibble.print_min = 30)
```

```{r}
#| label: res_summ_1-show
#| echo: !expr T
res_summ_1
```

```{r}
#| label: res_summ_1-show-post
...
```{r}
#| label: path_preds-appendix
path_preds <- file_path_out('preds_1.rds')
```

```{r}
#| label: do_predict-appendix-2
...
```{r}
#| label: path_viz_conf_mat
path_viz_conf_mat <- file_path_out('viz_conf_mat.png')
```

```{r}
#| label: viz_conf_mat-appendix
conf_mat_tidy <-
  preds_aug %>% 
...
```{r}
#| label: path_viz_g_mode
path_viz_g_mode <- file_path_out('viz_g_mode.png')
.lab_caption_n_gm <- glue::glue('Total games: 1140.')
```

```{r}
#| label: viz_g_mode-appendix
viz_g_mode <-
  preds_tidy_aug %>% 
...
```{r}
#| label: path_viz_g_mean
path_viz_g_mean <- file_path_out('viz_g_mean.png')
```

```{r}
#| label: viz_g_mean-appendix
viz_g_mean <-
  preds_tidy_aug %>% 
...
```{r}
#| label: path_viz_result_mode
path_viz_result_mode <- file_path_out('viz_result_mode.png')
```

```{r}
#| label: viz_result_mode-appendix
preds_tidy_res <-
  preds_tidy %>% 
  filter(key == 'res_mode') %>% 
  mutate_at(
...
```{r}
#| label: preds_eda-show-1-pre
.opt_old <- getOption('tibble.print_min')
options(tibble.print_min = Inf)
```

```{r}
#| label: preds_eda-show-1
preds_by_tm %>% 
  select(
    `Team` = tm_h,
    `# of Wins` = n,
