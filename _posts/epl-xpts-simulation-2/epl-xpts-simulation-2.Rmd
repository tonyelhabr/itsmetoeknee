...
tb_sim_placings <- understat_sim_placings |> 
  ...
  tab_footnote(
    ...
    footnote = 'Given the team\'s shot-by-shot xG for the entire season, the frequency at which the team finishes at a given spot in the final table in 10k simulations. Further right indicates higher in the table, closer to rank one; further left indicates lower in the table, closer to relegation.'
  ) |> 
  tab_header(
...
While the null result may be discouraging, the simulations are useful in and of themselves. They can be used to understand the distribution of outcomes for team in a given season, as seen in the table below.

![](sim_placings.png)

In fact, we can use the the relative proportion of simulations in which a given team ends up at a given placing (or "rank") to identify which actual end-of-season placings were the most unexpected.
