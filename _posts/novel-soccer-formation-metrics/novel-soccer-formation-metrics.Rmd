---
title: Quantifying symmetry and flow in soccer pass networks
description: 'Making up stats to look smart'
author:
  - name: Tony ElHabr
...
To put this into the context of soccer, here's the pass network and max cut numbers for [the match between Liverpool and Manchester City on October 3, 2021](https://www.fotmob.com/match/3609994/matchfacts/liverpool-vs-manchester-city). [^3]

[^3]: Unfortunately, max cuts can be difficult to visualize for graphs with lots of edges and/or nodes, so reporting just the quantity is often the best thing we can do. 🤷

![](C:/Users/antho/Documents/projects/sports_viz/47-formation_symmetry/network-game_id=1549604-max_cut_weighted_w_logo.png)

Eliakim et. al. argue that, in relation to soccer, maximum cuts surmises what is captured separately by various measures of centrality ([betweenness](https://en.wikipedia.org/wiki/Betweenness_centrality), [indegree and outdegree](https://en.wikipedia.org/wiki/Directed_graph#Indegree_and_outdegree), etc.).

This is what the distribution of maximum cut values looks like for the games evaluated. The average is around 55.

histogram

The season-level correlation between maximum cut and the expected goals (xG) difference is 0.55. (We use xG difference because it is more indicative of relative quality of play than potential alternative measures, such as time of possession and goal differential.)

But is that number big, or, dare I say, "significant"? Well, the correlation of xG differential with the relative proportion of shots is 0.9. And with the relative proportion of passes is 0.6. So yes, 0.55 is certainly non-trivial. Higher correlation means that we could potentially use whatever metric as it's own standalone measure of quality of play.
