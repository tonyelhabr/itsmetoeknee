...
xgd_rank_preds <- table_with_xgd |> 
  transmute(
    ...
    actual_rank,
    actual_pts,
    .pred = predict(xgd_rank_fit),
    .resid = .pred - actual_rank,
    xgd_unexpected_rank = row_number(desc(.resid)),
    inv_xgd_unexpected_rank = row_number(.resid)
  )
```
...
Nonetheless, we might argue in favor of the approach based on simulations using match probabilities. It can be used to tell us quantitatively how likely a team's placing was.[^5] (See the "Prob." column in the first table.) The regression approach can also be used to quantify likelihood; however, the approach is more brittle, dependent on statistical assumptions.

[^5]: We'd convert the standard residual of the regression to a tail probability. With R, one could use `pnrom()` with the standardized residual.
