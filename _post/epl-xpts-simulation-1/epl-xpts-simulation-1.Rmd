...
How does this work? Well, we use `poibin::dpoibin()` and some linear algebra to look at all combinations of makes and misses of the shots, treating xG as the Bernoulli trial success for each corresponding shot. we arrive at the interesting result that, despite the two teams total xG being equal (=0.6), team A has a slightly higher probability of winning. There have been plenty of [explanations](https://hockey-graphs.com/2018/12/19/some-people-were-wrong-on-twitter/) on this "quality vs. quantity" phenomenon, so I won't go into it in detail. Nonetheless, this simple example illustrates why it is important to translate xG into another form in order to get a better understanding of match results and the standings.

### Objectives

Ok, so we've gone over what expected points are and why they're important. Now we set out to do the following.

1.  **Calculate xPts from shot xG for multiple seasons of data.** We'll limit the scope to the 2020/2021 and 2021/2022 seasons for the English Premier League.[^2]
...
3.  **Compare predictions of actual season-long points using xPts that we derive from understat and fotmob xG.** In particular, we'll be interested to see if our conclusions here matches the conclusions for (2).

[^2]: We've limited the scope for several reasons: (1) fotmob only has complete xG data for the 2020/2021 and 2021/2022 seasons as of writing, (2) I didn't want to have to map team names across the two data sources for a ton of teams; and (3) of all league, I'm most interested in the EPL 😄.
...
team_mapping <- '../sports_viz/59-xg_xpoints/team_mapping.csv' |> readr::read_csv()
...
#> # … with 19,000 more rows
```

We can use `load_fotmob_match_details()` in a similar fashion.[^3]

[^3]: Note that there are three additional shots in the fotmob data. There's nothing you can really do about data discrepancies 🤷.
...
Alright, now the fun part. We functionalize the code from the example for calculating expected points and apply them to our data. First, we apply `poibin::dpoibin()` to each team in each match to estimate the probability of scoring 0, 1, 2, etc. up to the number of shots taken by the team.
...
#>  1    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.0180         0
#>  2    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.124          1
#>  3    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.298          2
#>  4    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.320          3
#>  5    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.172          4
#>  6    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.0543         5
#>  7    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.0110         6
#>  8    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.00150        7
#>  9    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.000141       8
#> 10    14086 2020/2021 2020-09-12 FALSE   Arsenal Fulham   0.00000917     9
#> # … with 20,520 more rows
```

Then we perform the permutation of shots and compare the permutations for the two teams in a match to analytically calculate the probabilities of winning, losing, and drawing.
...
#> # A tibble: 1,520 × 7
#>    date       team              opponent          prob_d…¹ prob_…² prob_…³  xpts
...
#> # … with 1,510 more rows, and abbreviated variable names ¹​prob_draw, ²​prob_win,
#> #   ³​prob_lose
```

If there was any doubt about the expected points calculation, note that understat offers xPts in their data. The mean absolute error of our calculation of xPts with there is \~0.02.
...
#> # A tibble: 1,520 × 8
#>    season    date       team            result   pts raw_xpts   xpts     xptsd
#>    <chr>     <date>     <chr>           <chr>  <int>    <dbl>  <dbl>     <dbl>
#>  1 2020/2021 2020-09-12 Arsenal         w          3   2.89   2.93   -0.0340  
#>  2 2020/2021 2020-09-12 Crystal Palace  w          3   1.48   1.48   -0.00108 
#>  3 2020/2021 2020-09-12 Fulham          l          0   0.0587 0.0405  0.0182  
#>  4 2020/2021 2020-09-12 Leeds United    l          0   0.0434 0.0440 -0.000577
#>  5 2020/2021 2020-09-12 Liverpool       w          3   2.93   2.93    0.00103 
#>  6 2020/2021 2020-09-12 Newcastle       w          3   2.05   2.03    0.0186  
#>  7 2020/2021 2020-09-12 Southampton     l          0   1.22   1.22   -0.000544
#>  8 2020/2021 2020-09-12 West Ham United l          0   0.700  0.717  -0.0171  
#>  9 2020/2021 2020-09-13 Everton         w          3   1.74   1.74    0.00598 
#> 10 2020/2021 2020-09-13 Leicester City  w          3   2.91   2.91    0.000614
...
### 2. Match predictive performance[^4]

[^4]: Using the adjective "predictive" is a little facetious, since we're not actually making predictions out-of-sample. Rather, we're using models based on xG to evaluate which xG data source better explains observed results.
...
Note that the win and loss correlations are identical. This is due to the symmetric nature of the data---we have two records for each match, one from each team's perspective.[^5]

[^5]: Home field advantage is treated as a feature instead of defined directly via columns, i.e. `home_team`, `home_score`, etc., which is good practice in general.
...
2.  the [brier skill score (BSS)](https://en.wikipedia.org/wiki/Brier_score#Brier_Skill_Score_(BSS)), treating the empirical proportion of the specified outcome as the reference.[^6]
3.  a [calibration plot](https://changhsinlee.com/python-calibration-plot/), grouping predictions into "buckets" at every 5%.

[^6]: Draws occur for 22.5% of matches in the data set, and wins and losses occur in 38.8% of matches each.
...
#> 1 d      fotmob    <tibble [10 × 5]> 0.170 0.0268
#> 2 d      understat <tibble [10 × 5]> 0.166 0.0466
#> 3 w      fotmob    <tibble [17 × 5]> 0.173 0.270 
#> 4 w      understat <tibble [17 × 5]> 0.162 0.317
```

Interestingly, the MSE (where lower is "better") and the BSS (where higher is "better") lead us to opposite conclusions regarding whether the understat or fotmob xG is better. The fotmob win and draw probability models are rated better by MSE, and the understat models are rated better by BSS. So what conclusion should we draw? Well, personally, I tend to rely on BSS wherever I can. Not only is it more interpretable---it's a number between 0 and 1, while MSE can take on any value, depending on the context---I like that it accounts for some baseline (in this case, the empirical proportion of wins and draws, depending on which outcome we were trying to predict).

Moreover, looking at the calibration plot, the understat model predictions seem to stick closer to the 45 degree marker, which represents the line of perfect calibration.
...
Alternatively, we could regress expected points on points, and use the [root mean squared error (RMSE)](https://en.wikipedia.org/wiki/Root-mean-square_deviation) and [R squared](https://en.wikipedia.org/wiki/Coefficient_of_determination)---both of which are common metrics used to evaluate regression models---to compare the two xG sources.
...
Personally, I don't like predicting points directly like this since it's a discrete variable that can only take on three values (0, 1, and 3).

#### Predicting match outcomes with multinomial logistic regression

If we're going to predict points instead of a probability, I think the better approach is to run a multinomial logistic regression and convert the predicted probabilities to expected points.
...
We see that understat has a lower RMSE and higher R squared with both approaches to predicting points (direct linear regression and post-processing of a multinomial logistic regression). The implication that understat performs slighlty better agrees with the first set of binary logistic regression models for predicting match outcome probabilities. Overall, we can say that understat seems to be the better of the two xG sources for explaining individual match results.

### 3. Season predictive performance

Ok, so how do the understat and fotmob models fare if we aggregate up the expected points to the season level and predict actual points?[^7]

[^7]: Note that aggregating match-level probabilities to the season-level is not really a valid way to use the probabilities, which are intended to be treated independently.
...
Alas, we find that understat's xG model seems to outperform fotmob's in terms of explaining match results and season-long point totals, so no one should ever use fotmob's site again! I am not one to shy away from calling out data providers on their B.S. I am glad to continue to live up to my reputation.
